...
class ChatifyMessenger
{
    ...
    public function messageCard($data, $viewType = null){
        if($viewType == 'request'){
            return view('Chatify::layouts.messageCardRequest',$data)->render();
        }else{
     ...
     * @return Collection
     */
    public function fetchMessagesQuery($user_id, $reference_id = 1){
        return Message::where('reference_id', $reference_id)
    ...
    public function newMessage($data){
        ...
        $message->reference_id = $data['reference_id'];
        $message->body = $data['body'];
        $message->attachment = $data['attachment'];
        $message->save();

        $userConnection = User_Connections::find($data['reference_id']);
        $userConnection->touch();
     ...
     * @return bool
     */
    public function makeSeen($user_id,$reference_id){
    ...
    public function getLastMessageQuery($user_id){
        return self::fetchMessagesQuery($user_id, 1)->orderBy('created_at','DESC')->latest()->first();
    }
