...
channel.bind('messaging', function (data) {
    console.info(data.from_id+' - '+data.to_id+' - '+data.reference_id+'\n'+auth_id+' - '+messenger);
    if (data.from_id == messenger.split('_')[1] && data.to_id == auth_id && data.reference_id == 1) {
        // remove message hint
        $(".message-hint").remove();
        // append message
 ...
 *-------------------------------------------------------------
 */
function makeSeen(status,referenceId) {
    // remove unseen counter for the user from the contacts list
    $('.messenger-list-item[data-contact=' + messenger.split('_')[1] + ']').find('tr>td>b').remove();
    // seen
    $.ajax({
