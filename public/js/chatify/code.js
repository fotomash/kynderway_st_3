...
function cssMediaQueries() {
 if (window.matchMedia('(min-width: 980px)').matches) {
     ...
     $('.messenger-listView').css('display','none');
 }
...
*-------------------------------------------------------------
*/
function IDinfo(id, type, reference_id) {
 console.log("id : " +id);
 ...
 if (messenger != 0) {
     ...
     $.ajax({
         ...
         success: (data) => {
             ...
             // fetch messages
             fetchMessages(id, type, reference_id);
...
*-------------------------------------------------------------
*/
function fetchMessages(id, type, reference_id) {
 if (messenger != 0) {
     $.ajax({
         url: url + '/fetchMessages',
         method: 'POST',
         data: { '_token': access_token, 'id': id, 'reference_id': reference_id, 'type': type },
         dataType: 'JSON',
         success: (data) => {
...
function closeChat(from_id,to_id) {
    if ($(".modal").is(":visible")) {
    ...
    console.log(from_id);
    console.log(to_id);
    return channel.trigger('client-close-chat', {
        from_id: from_id, // Me
        to_id: to_id, // Messenger
        closed: true,
        open: false,
    });
}
...
channel.bind('client-close-chat', function(dataChat) {
    isChatOpen = dataChat;
});

// Listen to messages, and append if data received
channel.bind('messaging', function (data) {
    if (data.from_id == messenger.split('_')[1] && data.to_id == auth_id && data.reference_id == reference_id) {
       // remove message hint
       $(".message-hint").remove();
       // append message
       ...
       console.log("test----------------------"+data.reference_id);

       console.log(isChatOpen);
       if (isChatOpen.open === undefined || isChatOpen.open === true) {
            makeSeen(true,data.reference_id);
       }
...
function makeSeen(status, reference_id) {
    // Remove unseen counter for the user from the contacts list
    $('.messenger-list-item[data-contact=' + messenger.split('_')[1] + ']').find('tr>td>b').remove();
    ...
    $.ajax({
        url: url + '/makeSeen',
        method: 'POST',
        data: { '_token': access_token, 'id': messenger.split('_')[1], reference_id: reference_id },
        dataType: 'JSON',
        success: (data) => {
...
function deleteConversation(id) {
 $.ajax({
     ...
     success: (data) => {
         ...
         // refresh info
         IDinfo(id, messenger.split('_')[0], messenger.split('_')[2]);
...
$(document).ready(function () {
 ...
 pusher.connection.bind('state_change', function (states) {
     ...
     channel.bind('pusher:subscription_succeeded', function () {
         ...
         IDinfo(messenger.split('_')[1], messenger.split('_')[0], messenger.split('_')[2]);
     });
 });
 ...
 $('.messenger-listView').css('width','0px');
...
$(document).ready(function () {
    ...
    $(document).on('click', '.btn-block-access-dropdown', function (e) {
        ...
        $.ajax({
            url: '/provider/documents/block-access',
            type: 'POST',
            data: {
    ...
    $(document).on('click', '.btn-grant-access-dropdown', function (e) {
        ...
        $.ajax({
            url: '/provider/documents/grant-access',
            type: 'POST',
            data: {
                document_id: documentId,
                seeker_id: seekerId,
